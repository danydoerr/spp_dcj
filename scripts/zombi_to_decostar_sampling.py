...
if len(parameters) < 4:
    print('usage: python zombi_to_decostar.py zombi_output_dir decostar_input_dir name rec/unrec boltzmann_temp boltzmann_samples')
    exit()
...
if recparam == 'unrec':
    ...
    print('Error')
boltzmann_temp = float(parameters[4])
if boltzmann_temp>0:
    boltzmann_samples=int(parameters[5])

parameter_file = open(decostar_dir + '/parameters_' + expname + '.txt',
                      'w')
...
species_tree = script_tree.readTree(open(zombi_dir + '/T/ExtantTree.nwk'
                                    , 'r').readline())
...
species_tree_output_file.write(script_tree.writeTree(species_tree,
                               root, False))
...
parameter_file.write('gene.distribution.file=list_gene_trees_'
                     + expname + '\n')
...
for file_name in genome_files:
    ...
    if species in all_species:
        file_genome = open(zombi_dir + '/G/Genomes/' + file_name, 'r'
                           ).readlines()
        i = 1
        while i < len(file_genome):
            ...
            dir2 = line_i1[2]
            if gene1 not in all_genes[species] \
                or gene2 not in all_genes[species]:
                print ('BUG BUG BUG', gene1, gene2,
                       all_genes[species].keys())
            adjacencies.write(gene1 + ' ' + gene2 + ' ' + dir1 + ' '
                              + dir2 + ' 1.0\n')
            i = i + 1
...
if boltzmann_temp==0:
    ...
    parameter_file.write('boltzmann.temperature='+str(boltzmann_temp)+'                 \n')
    parameter_file.write('nb.sample='+str(boltzmann_samples)+'              \n')
parameter_file.write('\n')
