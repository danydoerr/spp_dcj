...
def getNodes(tree):
    clefs = list(tree.keys())
    c = 0
    while c < len(clefs):
        if clefs[c] == 'sequence' or clefs[c] == 'ancestor' \
            or len(tree[clefs[c]]) == 0:
            del clefs[c]
        else:
            c = c + 1
    return clefs
...
def treeCopy(tree):
    ...
    for k in tree.keys():
        if k == 'ancestor' or k == 'sequence':
            ...
            result[k] = [ tree[k][0], tree[k][1], list(tree[k][2]), tree[k][3],
                    tree[k][4], tree[k][5], tree[k][6], ]
    return result
...
def changeRoot(tree, newRoot):  # the new root is between newRoot and its parent NEVER TESTED
    ...
    if not isRoot(tree, newRoot) and not isRoot(tree, getParent(tree,
            newRoot)):
        ...
        tree[getBrother(tree, current)][3] = tree[getBrother(tree,
                current)][3] + current_length
        del tree[root]
...
def unroot(tree):
    ...
    if len(nodes) > 3:
        ...
        if len(children) == 2:
            ...
            tree[children[1]][3] = tree[new_root][3] \
                + tree[children[1]][3]
            tree[children[1]][6] = max(tree[new_root][6],
                    tree[children[1]][6])
            tree[new_root][3] = -1
            del tree[root]
...
# print "begin",len(nodes)
    ...
    for n in nodes:
        if isfloat(tree[n][6]):
            ...
            tree[n][6] = 0.0
        if not isRoot(tree, n) and not isLeaf(tree, n) and tree[n][6] \
            < threshold:
...
def ultrametricize(tree):
    ...
    while len(marque) < len(getNodes(tree)):
        ...
        while not i in marque:
            distance_from_marque = distance_from_marque \
                + getLength(tree, i)
            i = getParent(tree, i)

                # print maximum,distance_from_marque,distanceFrom(tree,i,root),

        ratio = float(maximum - distanceFrom(tree, i, root)) \
            / distance_from_marque
...
# print "bip trouve"
                    ...
                    support[bipartitions[s]] = support[bipartitions[s]] \
                        + 1
...
def RF(arbre1, arbre2):
    ...
    for n in nodes1:
        ...
        if len(leaves) > 1:
            clade = []
            for l in leaves:
                clade.append(getName(arbre1, l).split('|')[0].split('__'
                             )[0])
                clade.sort()
    ...
    for n in nodes2:
        ...
        if len(leaves) > 1:
            clade = []
            for l in leaves:
                clade.append(getName(arbre2, l).split('|')[0].split('__'
                             )[0])
                clade.sort()
    ...
    for n1 in range(len(leaves1)):
        for n2 in range(n1 + 1, len(leaves1)):
            for n3 in range(n2 + 1, len(leaves1)):
                ...
                ids.sort(lambda x, y: cmp(getName(arbre1, x),
                         getName(arbre1, y)))
                names = [getName(arbre1, ids[0]), getName(arbre1,
                         ids[1]), getName(arbre1, ids[2])]
...
# print LCA12,LCA13,LCA23
    ...
    # print n1,n2,n3,result
                ...
                ids.sort(lambda x, y: cmp(getName(arbre2, x),
                         getName(arbre2, y)))
                names = [getName(arbre2, ids[0]), getName(arbre2,
                         ids[1]), getName(arbre2, ids[2])]
                if triplets.has_key('_'.join(names)):
                    ...
                    if LCA12 == LCA13 and triplets['_'.join(names)] \
                        == 1:

                # print names,"yes",triplets['_'.join(names)]

                        result = result + 1
                    elif LCA12 == LCA23 and triplets['_'.join(names)] \
                        == 2:

                # print names,"yes",triplets['_'.join(names)]

                        result = result + 1
                    elif LCA13 == LCA23 and triplets['_'.join(names)] \
                        == 3:
